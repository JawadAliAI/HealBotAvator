...
export function Avatar(props) {
  ...
  useFrame(() => {
    ...
    // LIP SYNC - Apply mouth movements based on audio timing
      ...
      if (!audio.paused && !audio.ended) {
        ...
        for (let i = 0; i < lipsync.mouthCues.length; i++) {
          ...
          if (currentAudioTime >= mouthCue.start && currentAudioTime <= mouthCue.end) {
            const visemeTarget = visemesMapping[mouthCue.value];

            // Debug: log lip sync activity (throttled to 1% of frames)
            if (import.meta.env.DEV && Math.random() < 0.01) {
              console.log(`👄 Lip Sync Active: time=${currentAudioTime.toFixed(2)}s, phoneme=${mouthCue.value}, viseme=${visemeTarget}`);
            }

            if (visemeTarget) {
              appliedMorphTargets.push(visemeTarget);
              lerpMorphTarget(visemeTarget, 1, 0.2);
            } else {
              console.warn(`⚠️ No viseme mapping for phoneme: ${mouthCue.value}`);
            }
            break;
          }
        }
      } else if (import.meta.env.DEV && Math.random() < 0.001) {
        // Very occasional logging when audio is not playing
        console.log(`⏸️ Audio state: paused=${audio.paused}, ended=${audio.ended}, time=${audio.currentTime.toFixed(2)}s`);
      }
    }
