...
export const ChatInterface = ({ hidden, ...props }) => {
  ...
  const sendMessage = () => {
    ...
    const userId = userIdInput.current.value || "demo_user_123"; // Default if empty
    if (!loading && !message) {
      tts(text, userId);
      input.current.value = "";
    }
  ...
  return (
    <div className="fixed top-0 left-0 right-0 bottom-0 z-10 flex justify-between p-4 flex-col pointer-events-none">
      <div className="self-start backdrop-blur-md bg-white bg-opacity-50 p-4 rounded-lg">
        <h1 className="font-black text-xl text-gray-700">Dr. HealBot 🩺</h1>
        <p className="text-gray-600">
          {loading ? "Thinking..." : "Your AI Medical Assistant - Type or speak your symptoms"}
        </p>
      </div>
