...
*   **Voice Interaction**: Speak to the avatar using the microphone (Speech-to-Text via Web Speech API).
...
*   **Retry Logic**: Automatic retries for slow or sleeping APIs.
...
3.  **Chat**:
    ...
    *   **Speak**: Click the **Microphone** button. Allow browser permissions if asked. Speak your message, and click the button again to stop.
...
The `chat` function accepts a `userId` to maintain session context and includes retry logic for reliability:
...
If you see this error occasionally, it's likely because:
- Your API is hosted on a free service (like Render) that goes to sleep when inactive
- The API takes 30-60 seconds to wake up on the first request
- Network connectivity issues

**Solutions:**
- Wait a moment and try again - the server is waking up
- Upgrade to a paid hosting plan to prevent sleeping
- The app has built-in retry logic that will automatically retry failed requests

### No Audio

- Check browser console (F12) for errors
- Ensure your speakers/volume are not muted
- Check that the backend is running on port 3000
- Verify your external TTS API is responding

### Voice Button Not Working

- Allow microphone permissions when prompted
- Use Chrome, Edge, or Safari (best compatibility)
